...
On `/page/<slug>` (the dual-mode editor, see [[q16-pages-lists-creates-slug-hosts-editor]]), clicking a block selected it with a ring and the full canvas stayed visible while config UI appeared alongside. This conflicted with chart interaction (selection/cross-filter click-through) and gave config a cramped surface.
...
- `src/routes/pages/[slug]/` (PageGrid focused mode; was `src/routes/page/[slug]/` before the route rename), verified 8/8 via CDP in the running app, PR #4
