...
This amends [[q16-pages-lists-creates-slug-hosts-editor]] ("dual-mode editor"): the editor is now **tri-mode**.
...
Block config is unchanged and stays where [[page-editor-block-config-focused-two-panel]] put it: ⚡ Bolt on a block → focused two-panel config via [[chartconfigdrawer-component]]. Block-level config is per-block (visual, contextual); *page-level* config is global (non-visual) → hence the tab split.
...
- Future page-level spec fields (filters, theme, shared color scale from [[q12-page-level-consistent-colors-shared-scale]]) have an obvious home: the Page tab.
- Design canvas is now full-width with no side rail at rest — block-config entry remains the ⚡ Bolt.

## Source

- `src/routes/pages/[slug]/+page.svelte` (was `src/routes/page/[slug]/` before the route rename) — tri-mode tabs, Page settings panel, sidebar removal
