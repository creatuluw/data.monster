...
- Cross-filtering runs through [[q8-one-canonical-query-engine-per-type-hooks]] — every chart re-queries on selection change; no client-side filtering layer to invent.
- With [[q7-relationship-graph-drives-item-availability]] auto-JOIN, a selection on one table's field filters charts joined from related tables via the relationship graph.
