...
5. **All content types in one build** (user: "no, do all" — no content-type phasing): pages, master items, relationships, saved queries, AND new `dm/connections.json` persistence (connections were never persisted before; optional plaintext password with warning).
