...
**Update — faster tell (same day, recurring report)**: user reported the cog "doesn't open a drawer"; the build had replaced the cog with the ⚡ bolt two commits earlier. When the user's **description of the UI itself** (icon name, label, menu item) doesn't exist in the current build, the webview is stale by definition — skip click-testing entirely and have them Ctrl+R first. Note: a real-trusted-click CDP pass then confirmed all three bolt buttons open the 50vw drawer (charts 16 fields, table 4 fields).
