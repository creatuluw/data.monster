...
> ⚠️ **SUPERSEDED — this conclusion was wrong.** CDP `Input.dispatchMouseEvent` DOES click svelteplot marks; the failures were stale coordinates, `element.click()` (no clientX for hit-testing), and an accidentally closed page target. See [[cdp-can-click-svelteplot-marks-dispatchmouseevent]].
