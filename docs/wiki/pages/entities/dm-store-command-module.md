...
- **Interface**:
  ...
  - Path→name inverses for the future watcher — `page_slug_from_path`, `item_from_path`, `saved_query_slug_from_path`
  ...
  - `check_json_object` — parse-level shape check the watcher will use to classify `dm:changed` vs `dm:error`
...
- Next: `files-002` remaps `list_pages`/`get_page`/`save_page`/`delete_page` onto it — `/pages` content becomes files
