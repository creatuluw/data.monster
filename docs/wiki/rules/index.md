...
- [Render markdown via marked + .prose-chat, never a new pipeline](./render-markdown-via-marked-prose-chat-never-a-new-pipeline.md) - When rendering any markdown anywhere in the app (docs tabs, chat, notes), parse with `marked` (already a dependency) and wrap the output in the `.prose-chat` cl
