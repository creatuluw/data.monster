<script lang="ts">
	...
	import { LineAreaChartCanvas } from '$lib/charts';
	import type { LineAreaChartConfig } from '$lib/charts/types';
	import { Settings, LineChart } from 'lucide-svelte';
	...
	let chartType: 'line' | 'area' = $state('line');
	let curve: 'linear' | 'monotone' = $state('monotone');
	let limit = $state(100);
	...
	let chartId = $state('chart-line-1');
	let clickedPoint: { category: string; value: number } | null = $state(null);

	let configReady = $derived(!!selectedTable && !!selectedDim && !!selectedMetric);

	let config = $derived<LineAreaChartConfig | null>(
		configReady
			? {
					id: chartId,
					table: selectedTable,
	...
	function handlePointClick(detail: { category: string; value: number }) {
		clickedPoint = detail;
		setTimeout(() => {
			clickedPoint = null;
		}, 2000);
...
<svelte:head>
	<title>Chart Library — Line/Area — Data Monster</title>
</svelte:head>
...
<div class="chart-detail">
	<div class="chart-panel">
		<div class="chart-panel-header">
			<div class="chart-panel-badge">
				<span class="section-number">CHART-002</span>
			</div>
		</div>
		<div class="chart-panel-body">
			{#if !config}
				<div class="chart-placeholder">
					...
					<p>Select a table, dimension, and metric to render a line or area chart.</p>
				</div>
			{:else}
				<LineAreaChartCanvas {config} onPointClick={handlePointClick} />
			{/if}
			{#if clickedPoint}
				<div class="bar-toast">
					<span class="bar-toast-cat">{clickedPoint.category}</span>
					<span class="bar-toast-val">{clickedPoint.value.toLocaleString()}</span>
				</div>
			{/if}
		</div>
	</div>
	...
	<aside class="drawer">
		...
		<div class="drawer-body">
			<div class="config-section">
				...
				{:else if selectedTable && tableColumns.length > 0}
					<div class="field">
						...
						<span class="field-hint">Pick an ordered or categorical column for the X axis</span>
					</div>
			...
			<div class="config-section">
				<h3 class="config-section-title">Chart Type</h3>

				<div class="orient-options">
					<button
						class="orient-btn"
						class:orient-btn-active={chartType === 'line'}
					...
					<button
						class="orient-btn"
						class:orient-btn-active={chartType === 'area'}
			...
			<div class="config-section">
				...
				<div class="field">
					<label class="field-label" for="chart-curve">Curve</label>
					<div class="orient-options">
						<button
							class="orient-btn"
							class:orient-btn-active={curve === 'monotone'}
							onclick={() => curve = 'monotone'}
						>
							Smooth
						</button>
						<button
							class="orient-btn"
							class:orient-btn-active={curve === 'linear'}
							onclick={() => curve = 'linear'}
						>
							Linear
						</button>
					</div>
				...
				<div class="field">
					...
					>
						<option value="ASC">Ascending (chronological)</option>
				...
				<div class="field">
					...
					<input
						...
						bind:value={limit}
					/>
					<span class="field-hint">Limits data points returned (default 500)</span>
				</div>
				...
				<label class="toggle-row">
					<div class="toggle-label">
						...
						<span class="toggle-hint">Click points to select categories; hover for tooltip</span>
					</div>
					<button
						class="toggle"
				...
				<label class="toggle-row">
					<div class="toggle-label">
						<span>Show dots</span>
						<span class="toggle-hint">Data point markers on the line</span>
					</div>
					<button
						class="toggle"
						class:toggle-on={showDots}
				...
				<label class="toggle-row">
					<div class="toggle-label">
						...
						<span class="toggle-hint">Display value labels on points</span>
					</div>
					<button
						class="toggle"
...
ORDER BY "{selectedDim}" {sortDirection}
