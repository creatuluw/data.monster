<script lang="ts">
	...
	import { PieDonutChartCanvas } from '$lib/charts';
	import type { PieDonutChartConfig } from '$lib/charts/types';
	import { Settings, PieChart } from 'lucide-svelte';
	...
	function handleSliceClick(detail: { category: string; value: number }) {
		clickedSlice = detail;
		setTimeout(() => {
			clickedSlice = null;
		}, 2000);
...
<svelte:head>
	<title>Chart Library — Pie/Donut — Data Monster</title>
</svelte:head>
	...
	<div class="chart-panel">
		<div class="chart-panel-header">
			<div class="chart-panel-badge">
				<span class="section-number">CHART-003</span>
			</div>
		</div>
		<div class="chart-panel-body">
			{#if !config}
				<div class="chart-placeholder">
					...
					<p>Select a table, dimension, and metric to render a pie or donut chart.</p>
				</div>
			{:else}
				<PieDonutChartCanvas {config} onSliceClick={handleSliceClick} />
			{/if}
			{#if clickedSlice}
				<div class="bar-toast">
					<span class="bar-toast-cat">{clickedSlice.category}</span>
					<span class="bar-toast-val">{clickedSlice.value.toLocaleString()}</span>
				</div>
			{/if}
		</div>
	</div>
		...
		<div class="drawer-body">
			<div class="config-section">
				...
				{:else if selectedTable && tableColumns.length > 0}
					<div class="field">
						<label class="field-label" for="chart-dim">Dimension (slices)</label>
						<select
							id="chart-dim"
							class="input input-mono"
							value={selectedDim}
						...
						<span class="field-hint">Pick a categorical column for slices</span>
					</div>

					<div class="field">
						<label class="field-label" for="chart-metric">Metric (slice size)</label>
						<select
							id="chart-metric"
							class="input input-mono"
			...
			<div class="config-section">
				...
				<div class="orient-options">
					<button
						class="orient-btn"
						class:orient-btn-active={chartType === 'pie'}
					...
					<button
						class="orient-btn"
						class:orient-btn-active={chartType === 'donut'}
			...
			<div class="config-section">
				...
				<div class="field">
					<label class="field-label" for="chart-limit">Max slices</label>
					<input
						id="chart-limit"
						type="number"
						class="input input-mono"
						...
						bind:value={limit}
					/>
					<span class="field-hint">Limits slices returned (default 20)</span>
				</div>
				...
				<label class="toggle-row">
					<div class="toggle-label">
						...
						<span class="toggle-hint">Click slices to select categories; hover for tooltip</span>
					</div>
					<button
						class="toggle"
				...
				<label class="toggle-row">
					<div class="toggle-label">
						...
						<span class="toggle-hint">Interactive color legend with togglable slices</span>
					</div>
					<button
						class="toggle"
				...
				<label class="toggle-row">
					<div class="toggle-label">
						...
						<span class="toggle-hint">Display value labels on slices</span>
					</div>
					<button
						class="toggle"
