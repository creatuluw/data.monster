<script lang="ts">
	...
	import { ScatterChartCanvas } from '$lib/charts';
	...
	let chartId = $state('chart-scatter-1');
	let clickedPoint: { x: number; y: number; group: string } | null = $state(null);
	let selectedRows: BarChartData[] = $state([]);

	let configReady = $derived(!!selectedTable && !!selectedX && !!selectedY && !!selectedGroup);
	...
	let config = $derived<ScatterChartConfig | null>(
		configReady
			? {
					...
					groupBy: { field: selectedGroup, label: selectedGroup },
					limit,
					pointSize,
					pointOpacity,
					showLegend,
...
<svelte:head>
	<title>Chart Library — Scatter — Data Monster</title>
</svelte:head>
	...
	<div class="chart-panel">
		<div class="chart-panel-header">
			<div class="chart-panel-badge">
				<span class="section-number">CHART-004</span>
			</div>
		</div>
		<div class="chart-panel-body">
			{#if !config}
				<div class="chart-placeholder">
					...
					<p>Select a table, X/Y numeric fields, and a dimension to render a scatter plot.</p>
				</div>
			{:else}
				<ScatterChartCanvas {config} onPointClick={handlePointClick} onSelectionChange={handleSelectionChange} />
			{/if}
			{#if clickedPoint}
				<div class="bar-toast">
					<span class="bar-toast-cat">{clickedPoint.group || 'Point'}</span>
					<span class="bar-toast-val">x: {clickedPoint.x.toLocaleString()}, y: {clickedPoint.y.toLocaleString()}</span>
				</div>
			{/if}
		</div>
	</div>
		...
		<div class="drawer-body">
			<div class="config-section">
				...
				{:else if selectedTable && tableColumns.length > 0}
					<div class="field">
						<label class="field-label" for="chart-x">X Axis (numeric)</label>
						<select
							id="chart-x"
							class="input input-mono"
							value={selectedX}
					...
					<div class="field">
						<label class="field-label" for="chart-y">Y Axis (numeric)</label>
						<select
							id="chart-y"
							class="input input-mono"
							value={selectedY}
					...
					<div class="field">
						<label class="field-label" for="chart-group">Dimension (color by)</label>
						<select
							id="chart-group"
							class="input input-mono"
							value={selectedGroup}
						...
						<span class="field-hint">Categorical column for color grouping</span>
					</div>
				{/if}
			</div>
			...
			<div class="config-section">
				...
				<div class="field">
					<label class="field-label" for="chart-size">
				...
				<div class="field">
					<label class="field-label" for="chart-opacity">
						Point opacity: {pointOpacity.toFixed(2)}
					</label>
					<input
						id="chart-opacity"
						type="range"
				...
				<label class="toggle-row">
					<div class="toggle-label">
						...
						<span class="toggle-hint">Color legend for group categories</span>
					</div>
						<button
							class="toggle"
				...
				<label class="toggle-row">
					<div class="toggle-label">
						...
						<span class="toggle-hint">Click points to select; drag to lasso</span>
					</div>
					<button
						class="toggle"
			...
			{#if config}
				<div class="config-section">
					...
					<pre class="sql-preview"><code>SELECT "{selectedX}", "{selectedY}", "{selectedGroup}"
FROM "{selectedTable}"
