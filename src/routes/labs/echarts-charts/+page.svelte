<script lang="ts">
	...
	const charts: ChartEntry[] = [
		{ id: 'bar', label: 'Bar Chart', desc: 'Compare values across categories with click-to-filter, tooltips, dimming, and responsive Canvas rendering powered by Apache ECharts.', icon: BarChart2, status: 'available' },
...
<svelte:head>
	<title>ECharts — Data Monster Labs</title>
</svelte:head>

<div class="lib-page">
	<div class="lib-header">
		<span class="section-number">ECHARTS</span>
	</div>

	<div class="lib-body">
		<div class="lib-intro">
			<h1 class="lib-title">Apache ECharts</h1>
			<p class="lib-desc">Production-grade charting library by Apache. Renders to Canvas/SVG with rich interactivity, built-in tooltip, legend, zoom, and click-to-filter powered by <code>echarts</code>.</p>
		</div>

		<div class="chart-grid">
			{#each charts as ct (ct.id)}
				{#if ct.status === 'available'}
					<a href="/labs/echarts-charts/{ct.id}" class="chart-card">
