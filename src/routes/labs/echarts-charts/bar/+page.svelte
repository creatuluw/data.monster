<script lang="ts">
	...
	const DIM_COLOR = 'rgba(180, 180, 180, 0.35)';
	...
	function buildChart() {
		if (!chartEl || chartData.length === 0 || !selectedDim) return;

		if (!chartInstance) {
			chartInstance = echarts.init(chartEl);
			chartInstance.on('click', (params: echarts.ECElementEvent) => {
				if (!clickToFilter || !selectedDim || params.componentType !== 'series') return;
		...
		const itemColors = categories.map((cat, i) => {
			if (hasFilter) {
				return selectedCategories.has(cat) ? PALETTE[i % PALETTE.length] : DIM_COLOR;
			}
		...
		const option: echarts.EChartsOption = {
			...
			xAxis: {
				...
				splitLine: { show: isHorizontal, lineStyle: { color: '#f0f0f0' } }
			},
			yAxis: {
...
<svelte:head>
	<title>ECharts Bar Chart — Data Monster</title>
</svelte:head>
	...
	<div class="chart-panel">
		<div class="chart-panel-header">
			<div class="chart-panel-badge">
				<span class="section-number">ECHARTS-001</span>
			</div>
		</div>
		<div class="chart-panel-body">
			...
			{:else}
				<div class="chart-content">
					...
					{/if}
					<div class="echarts-chart" bind:this={chartEl} style="height: {screenHeight()}px;"></div>
				</div>
			{/if}
		</div>
	</div>
	...
	<aside class="drawer">
		...
		<div class="drawer-body">
			<div class="config-section">
				...
				<div class="field">
					<label class="field-label" for="ec-table">Table</label>
					<select
						id="ec-table"
				...
				{:else if selectedTable && tableColumns.length > 0}
					<div class="field">
						<label class="field-label" for="ec-dim">Dimension (X axis)</label>
						<select
							id="ec-dim"
							class="input input-mono"
							value={selectedDim}
					...
					<div class="field">
						<label class="field-label" for="ec-metric">Metric (Y axis)</label>
						<select
							id="ec-metric"
							class="input input-mono"
			...
			<div class="config-section">
				...
				<div class="field">
					<label class="field-label" for="ec-orient">Orientation</label>
			...
			<div class="config-section">
				...
				<div class="field">
					<label class="field-label" for="ec-sort">Sort</label>
					<select
						id="ec-sort"
						class="input input-mono"
						value={sortDirection}
				...
				<div class="field">
					<label class="field-label" for="ec-limit">Max categories</label>
					<input
						id="ec-limit"
						type="number"
						class="input input-mono"
				...
				<div class="field">
					<label class="field-label" for="ec-fqid">Chart ID</label>
					<input
						id="ec-fqid"
						type="text"
						class="input input-mono"
...
LIMIT {limit}</code></pre>
			...
			<div class="config-section">
				...
				<p class="config-desc">
					...
					&mdash; Canvas-rendered bar chart with tooltip, click-to-filter, and dimming
				</p>
			</div>
		</div>
	</aside>
