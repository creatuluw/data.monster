<script lang="ts">
	...
	let chartId = $state('svelteplot-1');
	...
	let plotData = $derived(
		chartData.map((d, i) => ({
			category: String(d[selectedDim] ?? ''),
			value: typeof d.value === 'number' ? d.value : (Number(d.value) || 0),
			_index: i
		}))
	);

	function fillValue(_d: unknown, i: number): string {
		const d = plotData[i];
		if (!d) return PALETTE[i % PALETTE.length];
		if (clickToFilter && selectedCategories.size > 0) {
			return selectedCategories.has(d.category)
	...
	function handleBarClick(_event: Event, datum: { category: string }, _index: number) {
		if (!clickToFilter) return;
		const next = new Set(selectedCategories);
		if (next.has(datum.category)) {
			next.delete(datum.category);
		} else {
			next.add(datum.category);
		}
...
<svelte:head>
	<title>SveltePlot Bar Chart — Data Monster</title>
</svelte:head>
	...
	<div class="chart-panel">
		<div class="chart-panel-header">
			<div class="chart-panel-badge">
				<span class="section-number">SVLTEPLOT-001</span>
			</div>
		</div>
						...
						{#if orientation === 'horizontal'}
							...
							>
								<BarX
									...
									x="value"
									y="category"
									fill={{ scale: null, value: fillValue }}
									cursor={clickToFilter ? 'pointer' : undefined}
									onclick={clickToFilter ? handleBarClick : undefined}
								/>
							</Plot>
						{:else}
							<Plot
	...
	<aside class="drawer">
		...
		<div class="drawer-body">
			<div class="config-section">
				...
				<div class="field">
					<label class="field-label" for="sp-table">Table</label>
					<select
						id="sp-table"
				...
				{:else if selectedTable && tableColumns.length > 0}
					<div class="field">
						<label class="field-label" for="sp-dim">Dimension (X axis)</label>
						<select
							id="sp-dim"
							class="input input-mono"
							value={selectedDim}
					...
					<div class="field">
						<label class="field-label" for="sp-metric">Metric (Y axis)</label>
						<select
							id="sp-metric"
							class="input input-mono"
			...
			<div class="config-section">
				...
				<div class="field">
					<label class="field-label" for="sp-orient">Orientation</label>
			...
			<div class="config-section">
				...
				<div class="field">
					<label class="field-label" for="sp-sort">Sort</label>
					<select
						id="sp-sort"
						class="input input-mono"
						value={sortDirection}
				...
				<div class="field">
					<label class="field-label" for="sp-limit">Max categories</label>
					<input
						id="sp-limit"
						type="number"
						class="input input-mono"
				...
				<div class="field">
					<label class="field-label" for="sp-fqid">Chart ID</label>
					<input
						id="sp-fqid"
						type="text"
						class="input input-mono"
...
LIMIT {limit}</code></pre>
			...
			<div class="config-section">
				...
				<p class="config-desc">
					Powered by
					<a href="https://svelteplot.dev" target="_blank" rel="noopener noreferrer">SveltePlot</a>
					&mdash; Svelte-native grammar of graphics charting library
				</p>
			</div>
		</div>
	</aside>
