<script lang="ts">
	...
	// library components drive the + chart buttons — registered = available
...
<div class="page-shell" style="padding: var(--space-6);">
	...
	{:else}
		...
		<div class="space-y-4">
			...
			{#each libraryCharts as c (c.def.type)}
				<button class="px-3 py-1.5 border border-zinc-300 rounded-lg text-sm text-zinc-600 hover:bg-zinc-50" onclick={() => addBlock('chart', c.def.type)}>+ {c.def.label}</button>
			{/each}
